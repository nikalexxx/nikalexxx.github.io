...
${a.href('/?/physics/standard-model')('Реализация')} лежит в разделе ${a.href('/?/physics')('Физика')}.


${h(3)('Какие возможности использовались?')}
...
Так как по умолчанию сетка заполняется построчно(${code('grid-auto-flow: row;')}), то нам достаточно пропустить некоторое количество ячеек.
...
text-align: right;
`)}
${code('grid-column')} просто сокращенная запись для
${pre(`
grid-column-start: 1;
grid-column-end: 8;
`)}
...
`)}
Так как элементы с абсолютным позиционированием перекрывают остальные, а нам этого не надо, дополнительно выставим ${code('z-index: 2;')} для всех карточек, чтобы они отображались сверху, а цветная область выглядела как подложка.
`;
}

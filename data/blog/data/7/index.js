...
`)}
Выражение ${code(`@media (max-width: 700px)`)} означает, что правила внутри него будут применяться только когда максимальная ширина страницы 700px, то есть для любой ширины меньше 700px.


${code('.mobile-visible')} — класс на элементе ${code('html')}(${code('document.documentElement')}).
...
Кнопка, показывающая меню, просто переключает класс ${code('.mobile-visible')}
${pre(`

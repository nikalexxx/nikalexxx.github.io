...
Один из первых и точно самый популярный клеточный автомат в 1970 придумал ${a.href('https://en.wikipedia.org/wiki/John_Horton_Conway')('Джон Конвей')} и назвал его ${a.href('https://en.wikipedia.org/wiki/Conway%27s_Game_of_Life')('Игра Жизнь')}.
...
Также сегодня есть достаточно быстрая программа ${a.href('https://en.wikipedia.org/wiki/Golly_(program)')('Golly')}, которая оптимизирует расчёты.


${a.href('/?/projects/game-of-life')('Реализация')} лежит в разделе ${a.href('/?/projects')('Проекты')}. Вообще в раздел Проекты будут попадать все новые вещи, пока я не решу выделить им отдельное пространство имён.
...
Для каждого из состояний следующее состояние определяется суммой живых клеток в ${a.href('https://en.wikipedia.org/wiki/Moore_neighborhood')('окрестности Мура')}.
Размер клетки — 1 квадратный пиксель.
Поле расположено на ${code('canvas')}.
Показаны номер текущего поколения и количество живых клеток(популяция).


${h(3)('Что можно делать')}
...
Например, при переходе от ${code('B3/S23')} к ${code('B3/S234')}(нажмите галочку с цифрой 4 в нижнем ряду).


${h(3)('Начальные конфигурации')}
...
С ${a.href('https://www.conwaylife.com/wiki/Main_Page')('тематического сайта')} я отобрал наиболее интересные мне экземпляры.
По умолчанию выбрана самая сложная конфигурация из представленных.
Все конфигурации записаны в коде ${a.href('https://www.conwaylife.com/wiki/Run_Length_Encoded')('RLE')}, его также можно посмотреть, нажав галочку код конфигурации.
В будущем думаю добавить возможность задавать произвольные конфигурации в поле ввода.


${h(3)('Как устроено')}
Поле представлено массивом длиной 500000(1000*500) со значениями ${code('boolean')}.
${code('true')} соответствует живая клетка, ${code('false')} — мёртвая.
...
Затем конфигурация выставляется по центру в поле, в указанные координаты(со смещением относительно центра) выставляя ${code('true')}.


Каждый тик происходит ререндер.
Я хотел убедится, что ${a.href('/?/blog/3')('движок')}(ещё сырой) справится с нагрузкой.
...
Также были использованы новые иконки из бесплатного ${a.href('https://fontawesome.com/icons?d=gallery&m=free')('набора')} Font Awesome.


Для интересующихся всегда есть ${a.href('https://github.com/nikalexxx/nikalexxx.github.io/blob/master/components/GameOfLife/GameOfLife.js')('исходники')}.
`;
}

...
Они позволяют внутри js файлов использовать конструкции ${pre('export')} и ${pre('import')}, и браузер подгрузит все зависимости.
В теге script в html необходимо также указать, что скрипт используется в качестве модуля ${pre('&lt;srcipt src="example.js" type="module"></script>')}.
...
Я решил использовать ${link.href('https://lesscss.org')('Less')}, так как он обладает минимумом нужных функций(мне нужен только nesting и переменные).
...
${code.lang('javascript')('css(import.meta.url, \'./Example.less\')')}

Эта функция создавала элемент ${pre('<link>')}, его уже подхватывал скрипт less и генерировал стили.
В результате стили грузились после отрисовки контента и верстка "скакала".


${header.level(3)('Сборка')}
Стал использовать ${link.href('https://parceljs.org')('Parcel')} из-за того, что он многое поддерживает из коробки и практически не требует настроек.
...
Для разработки использую расширение ${link.href('https://marketplace.visualstudio.com/items?itemName=ritwickdey.LiveServer')('Live Server')} для VSCode.
`;
}

...
Посмотреть на конечный результат можно ${link.href('?/travels/altai')`здесь`}.


${header.level(3)`Подготовка изображений`}
...
Для каждого изображения я создал его маленькие копии (${pre`jpg`} и ${pre`webp`}), которые бы вписывались в прямоугольник 320x240 (ориентация сохраняется, то есть для вертикальных фото прямоугольник имел вид 240x320).


Так как сайт статический, все изображения должны быть получены заранее.
Я использовал возможности утилиты ${link.href('https://ffmpeg.org/')`ffmpeg`} для конвертации формата и масштабирования.
Помимо этого, я создал список изображений ${pre`list.js`} с указанием их имени, расширения и размеров.
...
Запишем значения уменьшенных сторон в ${pre`config.json`}
${code.lang('json')`
{
    "longSize": 320,
    "shortSize": 240
...
Для запуска использовал ${link.href('https://www.ruby-lang.org/ru/')`ruby`}. Создадим файл ${pre`generate.rb`}
...
gem 'dimensions'
`}
и установим зависимости ${pre`bundle install`}
...
${header.level(3)`Компонент галереи`}
...
Чтобы загрузить картинку для фона, предварительно проверим, поддерживает ли браузер ${link.href('https://ru.wikipedia.org/wiki/WebP')`webp`} изображения.
...
Я написал отдельный компонент для этого, реализацию можно посмотреть на ${link.href('?/design/components')`странице компонентов`}.

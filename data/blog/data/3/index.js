export default (api) => {
    const {book, external, text, block} = api;
    const {a, code, h} = text;
    const {pre} = block;
    return book`
...
Новые фреймворки вообще не используют виртуальное дерево, например ${a.href('https://svelte.dev')('Svelte')}.
...
Подробнее можно посмотреть в ${a.href('https://github.com/nikalexxx/nikalexxx.github.io/tree/master/utils')('репозитории')} на Github.


${h(3)('Использование')}
Этот сайт полностью работает на этом движке.
Для иллюстрации я показал использование с большим количеством элементов на странице — ${a.href('/?/projects/unicode')('отображение Юникода')}.

...
В итоге стали популярны фронтенд фрейворки, которые сами следили за обновлением DOM дерева, такие как ${a('https://reactjs.org')('React')}, ${a('https://angular.io')('Angular')} и ${a('https://vuejs.org')('Vue')}. React и Vue используют концепцию Virtual DOM. Мы держим в памяти облегченную аналог DOM в виде объектов, сравниваем их и при обнаружении изменений обновляем элементы на странице.


${h(3)('Что выбрать?')}
Фрейворки, не использующие полное сравнение виртуальных моделей, мне не нравились из-за того, что они требуют введения специфического dsl для условий и циклов.
В таких фрейворках верстку нельзя генерировать функцией.
...
Новые фрейворки вообще не используют виртуальное дерево, например ${a('https://svelte.dev')('Svelte')}.
...
При разрастании приложения объём сгенерированного кода превысит объём кода рантайм-машины фрейворка c virtual DOM.
...
Прокси E создаёт абстракцию над html элементом(есть ещё S для svg и M для MathML).
Прокси Component позволяет использовать такие абстракции, как свойства и состояние.


Когда состояние комопнента меняется, вычисляется diff между старым и новым объектом.

...
Мы можем добиться хорошей верстки документов с помощью ${pre`TEX`},
...
так как верстка на основе ${pre`TEX`} не адаптируется под размер экрана.
Различные лёгкие "книжные" форматы, такие как ${link.href('https://en.wikipedia.org/wiki/FictionBook')`fb2`},
...
Другие, например ${link.href('https://en.wikipedia.org/wiki/EPUB')`epub`}, пытались совместить всё сразу, семантику и произвольную верстку. Как следствие, epub слишком перегружен, никак не ограничивая содержимое своих контейнеров.


Нам важно получить широкие оформительские возможности в сочетании с адаптивной версткой.
И мир уже знает решение — ${pre`html`}.
Веб представляет нам безграничные возможности для создания интерфейсов и может служить основой для книжного формата,
нижним уровнем реализации. Веб как основу для своего книжного формата ${link.href('https://bureau.ru/books/manifesto/')('использует')} дизайнерское бюро Артёма Горбунова.
Понятно, что нам требуется абстракция над ${pre`html`},
чтобы определить основные компоненты формата.
Интересно, что исторически ${pre`html`} использовался как язык разметки документов, и только потом с развитием веба он стал использоваться для создания приложений в браузере — сложных сайтов.


Основным и по сути единственным языком для создания абстракций в вебе служит ${em`javascript`}.
Его мы и будем использовать для реализации.
В итоге книгу можно будет распространять как обычную ${pre`html`} страницу, через файловое представление.
Это значит, что её можно будет открывать локально в браузере даже без доступа к интернету, это будет обычный файл.
Но также легко можно будет встроить книгу в сайт с помощью ${em`javascript`}.
Также книгу можно экспортировать по желанию в другой формат, с полной или частичной поддержкой функций,
например простую документацию можно преобразовать в ${pre`markdown`}
...
${header.level(3)`Требования, которые мы хотим соблюсти`}
${start(list)}
${item`
...
Например, заголовок в ${pre`html`} будет преобразован в тег ${pre`<h1>`}
`}
${end(list)}

${header.level(3)`Возможности разметки`}
${start(list)}
${item`Тип текста: ${strong`выделение`}, ${em`акцент`}, ${pre`машинопись`}`}
${item`Изображения: ${image.src('../assets/images/favicon/favicon-32x32.png').alt('favicon').position('inline')}`}
${item`Медиа контент: видео и тд.`}
${item`Всплывающие подсказки`}
${item`Спойлеры`}
${item`Ссылки: ${link.href('https://www.wikipedia.org')('Wikipedia')}`}
${item`...`}
${item`
Внешние компоненты
${external.scope('parvis')(`({E}) => E.input.type\`color\``)}
...
${external.scope('parvis')(`({E}) => E.div.style\`height: 300px; max-height: 40vw; max-width: 700px\`(E.iframe
...
${header.level(3)`Возможности взаимодействия`}
По умолчанию для браузера, так как он позволяет реализовать всё это уже сейчас.
${start(list)}
${item`Закладки, + закладка по умолчанию — текущее место в книге`}
${item`Заметки`}
${item`Автоматическая проверка обновлений, если есть источник`}
${item`
...
${header.level(3)`Устройство разметки`}
...
Мета элементы в ${pre`meta`}.
Допускается стандартизация дополнительных пространств имён.


У каждого элемента есть стандартные аттрибуты, например ${pre`href`} у ссылки.
Также у каждого элемента есть атрибут ${pre`meta`} для несхематизированных данных.
...
`}
${area(...(['b', 'i', 'pre', 'sub', 'sup'] as const).map(elem => format[elem]` текст `))}


${header.level(3)`Текущая реализация`}

export default api => {
    const {book, h, i, b, a, code, ul, li, img, external, sup, sub, pre} = api;
    const minibook = api => {
        const {book, i} = api;
        return book`Книга как ${i`функция`}!`;
    };
    return book`
...
Различные лёгкие "книжные" форматы, такие как ${a('https://en.wikipedia.org/wiki/FictionBook')`fb2`},
...
Другие, например ${a('https://en.wikipedia.org/wiki/EPUB')`epub`}, пытались совместить всё сразу, семантику и произвольную верстку. Как следствие, epub слишком перегружен, никак не ограничивая содержимое своих контейнеров.
...
${ul.start}
${li} Тип текста: ${b`выделение`}, ${i`акцент`}, ${code`машинопись`}

${li} Изображения: ${img('../assets/images/favicon/favicon-32x32.png', 'favicon')}

${li} Медиа контент: видео и тд.
${li} Всплывающие подсказки
${li} Спойлеры
${li} Ссылки: ${a('https://www.wikipedia.org')('Wikipedia')}
${li} ...
${li}
Внешние компоненты
...
${ul.start}
${li} Закладки, + закладка по умолчанию — текущее место в книге
${li} Заметки
${li} Автоматическая проверка обновлений, если есть источник
${li}

...
export default (api: BookApi) => {
    const { book, start, end, format, math, code, label, list, item, area } = api;
    const { i, b} = format;
    const $ = math;
    const $$ = math.block();

    const {paragraph, theorem, problem, proof, A, B, n, printNote} = getCustomElements(api);
...
${theorem.key('n-sets')`
(${b`а`}) ${area.inline().key('theorem_n-sets.а')`Подмножество счётного множества конечно или счётно.`}

(${b`б`}) ${area.inline().key('theorem_n-sets.б')`Всякое бесконечное множество содержит счётное подмножество.`}

(${b`в`}) ${area.inline().key('theorem_n-sets.в')`Объединение конечного или счётного числа конечных или счётных множеств конечно или счётно.`}
`}
...
(${b`б`}) Сформулируйте и докажите аналогичное утверждение для букв «${code.inline()`T`}».
`}
...
Чтобы перейти отсюда к последовательностям нулей и единиц, можно воспользоваться ${label.ref('2^U-01')`приёмом`}, описанным ${printNote`на с. 12`}.

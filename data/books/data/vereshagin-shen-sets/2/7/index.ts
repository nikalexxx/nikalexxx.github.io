...
Поскольку мы предположили, что множество ${$`I`} имеет большую мощность, чем ${$`V`}, рекурсивное определение задаёт функцию не на всём ${$`I`}, а только на некотором начальном отрезке ${$`[0, i)`}, а в точке ${$`i`} рекурсивное правило не определено (${label.ref('theorem_recursive-rule')`теорема ${n.theorem('recursive-rule')}`}).
...
${theorem.key('linear-func')`Существует (всюду определённая) функция ${$`f: \\R \\to \\R`}, для которой ${$`f(x + y) = f(x) + f(y)`} при всех ${$`x`} и ${$`y`}, но которая не есть умножение на константу.`}
...
${problem`Покажите, что всякая функция, обладающая указанными в ${label.ref('theorem_linear-func')`теореме ${n.theorem('linear-func')}`} свойствами, не ограничена ни на каком отрезке и, более того, её график всюду плотен в ${$`\\R^2`}.`}


${theorem`Аддитивные группы ${$`\\R`} и ${$`\\R \\oplus \\R`} изоморфны.`}
...
${problem`
Докажите, что любой базис в пространстве ${$`\\R`} над полем ${$`\\Bbb{Q}`} имеет мощность континуума.
...
${problem`
Покажите, что некоторое усложнение этого рассуждения позволяет обойтись без базиса Гамеля: достаточно определять ${$`\\varphi`} не на всех действительных числах, а только на линейных комбинациях углов, встречающихся при разрезании куба и тетраэдра на части.
`}

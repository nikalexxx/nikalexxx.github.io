...
const path = '/data/books/shen-sets/1/5';
...
export default (api) => {
    ...
    return _`
${paragraph('1.5. Теорема Кантора – Бернштейна')}
...
${problem(api)} Некто предложил такое определение: множество ${A} имеет строго меньшую мощность, чем множество ${B}, если оно равномощно некоторой части множества ${B}, не совпадающей со всем ${B}.
Почему это определение неудачно?
(Указание. Популярные рассказы о теории множеств часто начинаются с такого парадокса, восходящего к Галилею.
...
(Это вовсе не очевидное утверждение составляет содержание теоремы Кантора – Бернштейна, которую мы сейчас докажем.)
${end(li)}
${start(li)}
...
(Доказательство этого факта требует так называемой «трансфинитной индукции»; ${draft`см. раздел 2.6, теорема 25`}.)
${end(li)}
${end(ul)}

${start(area.key('teor_6'))}
${b`Теорема 6 (Кантора – Бернштейна)`}. Если множество ${A} равномощно некоторому подмножеству множества ${B}, а ${B} равномощно некоторому подмножеству множества ${A}, то множества ${A} и ${B} равномощны.
${end(area)}
...
Однако этого обещания он не выполнил, и первые доказательства были даны Шрёдером (1896) и Бернштейном (1897).
Как видно из работ и писем Кантора, он предполагал доказывать эту теорему одновременно с возможностью сравнить любые два множества (${draft`см. раздел 2.6, теорема 25`});
быть может, он имел в виду рассуждение, намеченное нами в задаче ${draft`117`}.
(Работы Кантора по теории множеств и его письма переведены на русский язык ${draft`[4]`};
...
${problem(api)} Посмотрите на приведённые выше задачи, где требовалось доказать равномощность, и убедитесь, что во многих из них применение теоремы Кантора – Бернштейна сильно упрощает дело.


${problem(api)} Докажите, что все геометрические фигуры, содержащие хотя бы кусочек прямой или кривой, равномощны.


${problem(api)} Докажите, что если квадрат разбит на два множества, то хотя бы одно из них равномощно квадрату.
...
${problem(api)} Докажите, что если отрезок разбит на две части, то хотя бы одна из них равномощна отрезку.
...
${problem(api)} Пусть ${$`f`} — взаимно однозначное соответствие между ${A} и некоторым подмножеством множества ${B}, а ${$`g`} — взаимно однозначное соответствие между ${B} и некоторым подмножеством множества ${A}.
...
${problem(api)} Докажите, что квадрат можно разбить на две части так, что из подобных им частей можно сложить круг. Формально: квадрат можно разбить на две части ${$`A'`} и ${$`A''`}, а круг — на две части ${$`B'`} и ${$`B''`}, для которых ${$`A'`} подобно ${$`B'`}, а ${$`A''`} подобно ${$`B''`}.
...
Этот случай на самом деле невозможен, но мы этого пока не знаем (см. раздел ${draft`2.6`}).
`)}
${end(ul)}

${problem(api)} Докажите, что счётное множество имеет меньшую мощность, чем любое несчётное.


${problem(api)} Проверьте аккуратно, что если ${A} имеет меньшую мощность, чем ${B}, а ${B} имеет меньшую мощность, чем ${C}, то ${A} имеет меньшую мощность, чем ${C} (транзитивность сравнения мощностей).
...
элементами.
Их насколько много, что образовать из них множество затруднительно, это может привести к парадоксам (${draft`см. раздел 1.6, с. 29`}).
...
${$`|A| \\leqslant |B|`} означает, что ${A} равномощно некоторому подмножеству множества ${B}, а ${$`|A| < |B|`} означает, что ${A} имеет меньшую мощность, чем ${B} (${draft`см. с. 24`}).
`;
};

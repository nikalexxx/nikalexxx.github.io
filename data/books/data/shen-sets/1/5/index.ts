...
export default (api) => {
    ...
    const {paragraph, problem, theorem, proof, A, B, C, n }= getCustomElements(api);
...
элементами.
Их насколько много, что образовать из них множество затруднительно, это может привести к ${label.ref('russell-paradox')`парадоксам`}.
...
${$`|A| \\leqslant |B|`} означает, что ${A} равномощно некоторому подмножеству множества ${B}, а ${$`|A| < |B|`} означает, что ${A} имеет ${label.ref('set-size<')`меньшую мощность`}, чем ${B}.
`;
};

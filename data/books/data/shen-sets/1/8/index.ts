...
export default (api) => {
    ...
    return _`
${paragraph('1.8. Операции над мощностями')}
...
${problem(api)} Чему равно ${$`0^0`} согласно нашему определению? (Ответ: единице.)
...
Гипотеза континуума, о которой мы упоминали на ${draft`с. 30`}, утверждает, что ${$`\\frak{c} = \\alef_1`}.)
...
${problem(api)} Объясните подробно выкладку:
${$$`
\\frak{c} + \\frak{c} = 1 \\times \\frak{c} + 1 \\times \\frak{c} = 2 \\times \\frak{c} = 2^1 \\times 2^{\\alef_0} = 2^{1 + \\alef_0} = 2{\\alef_0} = \\frak{c}.
`}


${problem(api)} Проверьте, что ${$`\\alef_0 \\times \\frak{c} = \\frak{c}`}.


Приведённые нами свойства мощностей полезно сочетать с ${label.ref('teor_6')`теоремой Кантора–Бернштейна`}.
...
${problem(api)} Последнее рассуждение неявно использует монотонность операции возведения в степень для мощностей
...
${problem(api)} Установите явное соответствие между последовательностями натуральных чисел и иррациональными числами на отрезке ${$`(0, 1)`}, используя цепные дроби, то есть дроби вида ${$`1 / (n_0 + 1 / (n_1 + 1 / (n_2 + \\dots)))`}.


${problem(api)} Проверьте, что ${$`\\frak{c}^{\\frak{c}} = \\alef_0^{\\frak{c}} = 2^{\\frak{c}}`}.
(Напомним, что по теореме Кантора эта мощность больше мощности континуума.)


${problem(api)} Какова мощность множества всех непрерывных функций с действительными аргументами и значениями?
Существенна ли здесь непрерывность?


${problem(api)} Какова мощность множества всех монотонных функций с действительными аргументами и значениями?


${problem(api)} Может ли семейство подмножеств натурального ряда быть несчётным, если любые два его элемента имеют конечное пересечение? конечную симметрическую разность?
...
Поэтому в задачах ${draft`47, 48`} нам пришлось воспользоваться обходным манёвром, чтобы доказать, что из ${$`a + b = \\frak{c}`} следует ${$`a = \\frak{c}`} или ${$`b = \\frak{c}`}.
Следующее утверждение обобщает этот приём:


${start(area.key('teor_10'))}
${b`Теорема 10`}. Если множество ${$`A_1 \\times A_2 \\times \\dots \\times A_n`} разбито на непересекающиеся части ${$`B_1, \\dots , B_n`}, то найдётся такое ${$`i`}, при котором мощность ${$`B_i`} не меньше мощности ${$`A_i`}.
${end(area)}
...
Заметим, что в формулировке этого утверждения (которое иногда называют теоремой Кёнига) говорится о декартовом произведении конечного числа множеств, которое можно определить индуктивно (скажем, ${$`A \\times B \\times C`} будет состоять из троек ${$`\\lang a, b, c \\rang`}, которые суть пары ${$`\\lang \\lang a, b \\rang , c \\rang `}).
...
После такого определения ${label.ref('teor_10')`теорема 10`} легко переносится и на счётные (а также и на любые) произведения.
...
${problem(api)} Докажите подробно это утверждение.


${problem(api)} Пусть ${$`a_0, a_1, a_2, \\dots`} — мощности, причём ${$`a_i > 2`} для всех ${$`i`}.
Покажите, что
${$$`
a_0 + a_1 + a_2 + \\ldots \\leqslant a_0 \\times a_1 \\times a_2 \\times \\ldots
`}
${problem(api)} Пусть ${$`m_0 < m_1 < m_2 < \\dots`} — возрастающая последовательность мощностей.

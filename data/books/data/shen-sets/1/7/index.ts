...
(К вопросу о том, что такое упорядоченная пара, мы ещё вернёмся на ${draft`с. 36.`})
...
${problem(api)}  Вопрос для самоконтроля: отношения «быть делителем» и «делиться на» — это одно и то же отношение или разные?
...
${$$`
\\{\\lang a, c \\rang \\ | \\ \\lang a, b \\rang \\in f \\ и \\ \\lang b, c \\rang \\in g \\ для \\ некоторого \\ b \\in B\\}.
`}
...
${$$`
f(A') = \\{f(a) \\ | \\ a \\in A'\\} = \\{b \\in B \\ | \\ \\lang a, b \\rang \\in f \\ для \\ некоторого \\ a \\in A'\\}.
`}
...
${problem(api)} Какие из следующих равенств верны?
${$$`
\\begin{aligned}
...
${problem(api)} Докажите, что функция ${$`f: A \\to B`} является вложением тогда и только тогда, когда она имеет ${i`левую обратную`} функцию ${$`g: B \\to A`}, то есть функцию ${$`g`}, для которой ${$`g \\circ f = id_A`}.
...
${problem(api)} Докажите, что функция ${$`f: A \\to B`} является вложением тогда и только тогда, когда на неё можно сокращать слева: из равенства ${$`f \\circ g_1 = f \\circ g_2`} следует равенство ${$`g_1 = g_2`}
...
${problem(api)} Могут ли для некоторой функции левая и правая обратные существовать, но быть различны?
...
(Здесь снова используется аксиома выбора, о которой мы говорили на ${draft`с. 14`}.)
...
${problem(api)} Найдите ошибку в этом рассуждении, не читая дальше.
...
${$$`
\\lang x_1, y_1 \\rang = \\lang x_2, y_2 \\rang \\lrArr x_1 = x_2 \\ и \\ y_1 = y_2.
`}
В принципе, можно так и считать понятие упорядоченной пары неопределяемым, а это свойство — аксиомой.
Однако при формальном построении теории множеств удобно использовать трюк, придуманный польским математиком Куратовским, и избежать появления отдельного понятия упорядоченной пары.
...
${start(area.key('teor_9'))}
${b`Теорема 9 (Упорядоченная пара по Куратовскому)`}.
...
${problem(api)} Докажите утверждение ${label.ref('teor_9')`теоремы 9`} для ${i`упорядоченной пары по Винеру`}: ${$`\\lang x, y \\rang = \\{\\{${empty}, \\{x\\}\\}, \\{\\{y\\}\\}\\}`}.
`;
};

...
${start(area.key('teor_7'))}
${b`Теорема 7 (Кантора)`}. Множество бесконечных последовательностей нулей и единиц несчётно.
${end(area)}
...
Первый пример такого числа построил в 1844 году французский математик Ж. Лиувилль.
...
Отметим кстати, что в том же 1874 году французский математик Ш. Эрмит доказал, что основание натуральных логарифмов ${$`e = 2,71828\\mathellipsis`} трансцендентно, а через восемь лет немецкий математик Ф. Линдеман доказал трансцендентность числа ${$`\\pi = 3,1415\\mathellipsis`} и тем самым невозможность квадратуры круга.


В нескольких следующих задачах мы предполагаем известными некоторые начальные сведения из курса математического анализа.


${problem(api)} Покажите, что для всякого несчётного множества ${$`A \\sub \\R`} можно указать точку ${$`a`}, любая окрестность которой пересекается с ${A} по несчётному множеству.
(Утверждение остаётся верным, если слова «несчётное множество» заменить на «множество мощности континуума».)


${problem(api)} Покажите, что любое непустое замкнутое множество ${$`A \\sub \\R`} без изолированных точек имеет мощность континуума.


${problem(api)} Покажите, что любое замкнутое множество ${$`A \\sub \\R`} либо конечно, либо счётно, либо имеет мощность континуума.
...
(Кантор, доказавший этот факт, рассматривал его как первый шаг к доказательству гипотезы континуума для общего случая, но из этого ничего не вышло.)


${problem(api)} Из плоскости выбросили произвольное счётное множество точек.
...
${start(area.key('teor_8'))}
${b`Теорема 8 (общая формулировка теоремы Кантора)`}. Никакое множество ${$`X`} не равномощно множеству всех своих подмножеств.
${end(area)}
...
Поэтому, вспоминая определение сравнения множеств по мощности (${draft`с. 24`}), можно сказать, что мощность множества ${$`X`} всегда меньше мощности множества ${$`P(X)`}


${problem(api)} Докажите, что ${$`n < 2^n`} для всех натуральных ${$`n = 0, 1, 2, \\dots`}


В общей формулировке ${label.ref('teor_8')`теорема 8`} появляется в работе Кантора 1890/91 года.
Вместо подмножеств Кантор говорит о функциях, принимающих значения 0 и 1.
...
Это рассуждение можно развернуть, вспомнив доказательство теоремы Кантора — получится так называемый парадокс Рассела.
...
Например, в работе Кантора 1878 года была сформулирована континуум-гипотеза: всякое подмножество отрезка либо конечно, либо счётно, либо равномощно всему отрезку.
...
Мы можем считать «пятый постулат Евклида» (через данную точку проходит не более одной прямой, параллельной данной) истинным.
...
[Отметим, кстати, распространённое заблуждение: почему-то широкие массы писателей о науке и даже отдельные математики в минуты затмений (см. статью в Вестнике Академии Наук, посвящённую юбилею Лобачевского) считают, что в неевклидовой геометрии параллельные прямые пересекаются.
...
Безопасные (по крайней мере пока не приведшие к противоречию) правила обращения со множествами сформулированы в аксиоматической теории множеств (формальная теория ZF, названная в честь Цермело и Френкеля).

...
export default (api) => {
    ...
    const {paragraph, theorem, proof, problem, A, B, n} = getCustomElements(api);
...
Поэтому, вспоминая ${label.ref('set-size-variants')`определение`} сравнения множеств по мощности, можно сказать, что мощность множества ${$`X`} всегда меньше мощности множества ${$`P(X)`}

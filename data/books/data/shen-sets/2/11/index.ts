...
export default (api) => {
    ...
    return _`
${paragraph('2.11. Арифметика ординалов')}
Мы определили сумму и произведение линейно упорядоченных множеств в ${draft`разделе 2.1`}.
...
Это подмножество не является начальным отрезком, но мы можем воспользоваться ${label.ref('teor_37')`теоремой 37`}.)
`)}
${li(_`
...
Это подмножество не является начальным отрезком, но можно сослаться на ${label.ref('teor_37')`теорему 37`}.)
`)}
${li(_`
...
${problem(api)} Для каких ординалов ${$`1 + \\alpha = \\alpha`}?


${problem(api)} Для каких ординалов ${$`2 \\cdot \\alpha = \\alpha`}?


${problem(api)} Какие ординалы представимы в виде ${$`\\omega \\cdot \\alpha`}?


${problem(api)} Докажите, что ${$`\\alpha + \\beta = \\beta`} тогда и только тогда, когда ${$`\\alpha\\omega \\leqslant \\beta`} (здесь ${$`\\alpha`} и ${$`\\beta`} — ординалы).


${problem(api)} Докажите, что если ${$`\\alpha + \\beta = \\beta + \\alpha`} для некоторых ординалов ${$`\\alpha`} и ${$`\\beta`}, то найдётся такой ординал ${$`\\gamma`} и такие натуральные числа ${$`m`} и ${$`n`}, что ${$`\\alpha = \\gamma m`} и ${$`\\beta = \\gamma n`}.


${problem(api)} Определим операцию «замены основания» с ${$`k > 1`} на ${$`l > k`}.

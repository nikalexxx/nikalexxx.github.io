...
export default (api) => {
    ...
    return _`
${paragraph('2.5. Трансфинитная индукция')}
...
Доказательства по индукции мы уже обсуждали, говоря о фундированных множествах (см. ${draft`раздел 2.3`}), и сейчас ограничимся только одним примером.
${start(area.key('teor_17'))}
${b`Теорема 17`}. Пусть множество ${A} вполне упорядочено, а отображение ${$`f: A \\to A`} возрастает
...
${$`${teor_start}`}
Согласно принципу индукции (${label.ref('teor_15')`теорема 15`}) достаточно доказать неравенство ${$`f(x) > x`}, предполагая, что ${$`f(y) > y`} при всех ${$`y < x`}.
...
${problem(api)} Какую функцию ${$`f`} задаёт такое определение?
...
${start(area.key('teor_18'))}
${b`Теорема 18`}. Пусть ${A} — вполне упорядоченное множество.
...
${problem(api)} Докажите, что множество всех таких «исключительных» ${$`a`} (когда последовательность конечна) счётно.


Аналогичная ситуация возможна и для общего случая.


${start(area.key('teor_19'))}
${b`Теорема 19`}. Пусть отображение ${$`F`}, о котором шла речь в ${label.ref('teor_18')`теореме 18`}, является частичным
...
${$`${teor_start}`}
Это утверждение является обобщением, но одновременно и следствием предыдущей ${label.ref('teor_18')`теоремы 18`}.
...
Применив ${label.ref('teor_18')`теорему 18`} к модифицированному правилу, получим некоторую функцию ${$`f'`}.
...
${problem(api)} Сформулируйте и докажите утверждение об однозначности функции, заданной частичным рекурсивным правилом.


Теперь у нас всё готово для доказательства теоремы о сравнении вполне упорядоченных множеств.


${start(area.key('teor_20'))}
${b`Теорема 20`}. Пусть ${A} и ${B} — два вполне упорядоченных множества.
...
Применяя ${label.ref('teor_19')`теорему 19`}, мы получаем функцию ${$`f`}, согласованную с этим правилом.
...
${start(area.key('teor_21'))}
${b`Теорема 21`}. Никакое вполне упорядоченное множество не изоморфно своему начальному отрезку (не совпадающему со всем множеством).
${end(area)}


${$`${teor_start}`}
Пусть вполне упорядоченное множество ${A} изоморфно своему начальному отрезку, не совпадающему со всем множеством.
Как мы видели на ${draft`с. 58`}, этот отрезок имеет вид ${$`[0, a)`} для некоторого элемента ${$`a \\in A`}.
Пусть ${$`f: A \\to [0, a)`} — изоморфизм.
Тогда ${$`f`} строго возрастает, и по ${label.ref('teor_17')`теореме 17`} имеет место неравенство ${$`f(a) > a`}, что противоречит тому, что множество значений функции ${$`f`} есть ${$`[0, a)`}.
${$`${teor_end}`}
...
Если ${A} изоморфно начальному отрезку множества ${B}, не совпадающему со всем ${B}, то говорят, что ${i(_`порядковый тип множества ${A} меньше порядкового типа множества ${B}`)}.
Если множества ${A} и ${B} изоморфны, то говорят, что у них ${i`одинаковые порядковые типы`}.
Наконец, если ${B} изоморфно начальному отрезку множества ${A}, то говорят, что ${i(_`порядковый тип множества ${A} больше порядкового типа множества ${B}`)}.
Мы только что доказали такое утверждение:


${start(area.key('teor_22'))}
${b`Теорема 22`}. Для любых вполне упорядоченных множеств ${A} и ${B} имеет место ровно один из указанных трёх случаев.
${end(area)}


${problem(api)} Пусть ${A} — вполне упорядоченное множество, а ${B} — его подмножество с индуцированным порядком
...
(Указание. Если ${A} изоморфно собственному начальному отрезку множества ${B}, нарушается ${label.ref('teor_17')`теорема 17`}.)
...
${start(area.key('teor_23'))}
${b`Теорема 23`}. Всякое непустое семейство вполне упорядоченных множеств имеет «наименьший элемент» — множество, изоморфное начальным отрезкам всех остальных множеств.
${end(area)}
...
${problem(api)} Покажите, что для любого вполне упорядоченного множества ${A} существует равномощное ему вполне упорядоченное множество ${B} с таким свойством: любой начальный отрезок ${B} (кроме всего ${B}) имеет меньшую мощность, чем ${B}.
(Множества с этим свойством — точнее, их порядковые типы — называют ${i`кардиналами`}.)


Из ${label.ref('teor_22')`теоремы 22`} следует, что любые два вполне упорядоченных множества сравнимы по мощности (одно равномощно подмножеству другого).
Сейчас мы увидим, что всякое множество может быть вполне упорядочено (${label.ref('teor_24')`теорема Цермело`}), и, следовательно, любые два множества сравнимы по мощности.
`;
};

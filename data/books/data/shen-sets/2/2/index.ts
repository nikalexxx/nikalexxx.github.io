...
export default (api) => {
    ...
    return _`
${paragraph('2.2. Изоморфизмы')}
Два частично упорядоченных множества называются ${i`изоморфными`}, если между ними существует ${$`изоморфизм`}, то есть взаимно однозначное соответствие, сохраняющее порядок.
...
${start(area.key('teor_12'))}
${b`Теорема 12`}. Конечные линейно упорядоченные множества из одинакового числа элементов изоморфны.
${end(area)}
...
${problem(api)} Докажите, что множество всех целых положительных делителей числа ${$`30`} с отношением «быть делителем» в качестве отношения порядка изоморфно множеству всех подмножеств множества ${$`\\{a, b, c\\}`}, упорядоченному по включению.


${problem(api)} Будем рассматривать финитные последовательности натуральных чисел, то есть последовательности, у которых все члены, кроме конечного числа, равны ${$`0`}.
...
${problem(api)} Покажите, что не существует автоморфизма упорядоченного множества ${$`\\N`} натуральных чисел, отличного от тождественного.


${problem(api)} Рассмотрим множество ${$`P(A)`} всех подмножеств некоторого ${$`k`}-элементного множества ${A}, частично упорядоченное по включению.
Найдите число автоморфизмов этого множества.


${problem(api)} Покажите, что множество целых положительных чисел, частично упорядоченное отношением «${$`x`} делит ${$`y`}», имеет континуум различных автоморфизмов.


Вот несколько примеров равномощных, но не изоморфных линейно упорядоченных множеств
(в силу ${label.ref('teor_12')`теоремы 12`} они должны быть бесконечными).
${start(ul)}
${start(li)}
...
${problem(api)} Докажите, что линейно упорядоченные множества ${$`\\Z \\times \\N`} и ${$`\\Z \\times \\Z`}
(с описанным выше на ${draft`с. 46`} порядком) не изоморфны.


${problem(api)} Будут ли изоморфны линейно упорядоченные множества ${$`\\N \\times \\Z`} и ${$`\\Z \\times \\Z`}?


${problem(api)} Будут ли изоморфны линейно упорядоченные множества ${$`\\Bbb{Q} \\times \\Z`} и ${$`\\Bbb{Q} \\times \\N`}?
...
${problem(api)} Покажите, что множество рациональных чисел интервала ${$`(0, 1)`} и множество ${$`\\Bbb{Q}`} изоморфны.
...
Более сложная конструкция требуется в следующей задаче (видимо, ничего проще, чем сослаться на общую ${label.ref('teor_13')`теорему 13`}, тут не придумаешь).


${problem(api)} Докажите, что множество двоично-рациональных чисел интервала ${$`(0, 1)`} изоморфно множеству ${$`\\Bbb{Q}`}.
...
${start(area.key('teor_13'))}
${b`Теорема 13`}. Любые два счётных плотных линейно упорядоченных множества без наибольшего и наименьшего элементов изоморфны.
${end(area)}
...
${problem(api)} Сколько существует неизоморфных счётных плотных линейно упорядоченных множеств (про наименьший и наибольший элементы ничего не известно)? (Ответ: 4.)


${problem(api)} Приведите пример неизоморфных линейно упорядоченных множеств мощности c без наименьшего и наибольшего элементов.
(Указание: возьмите множества ${$`\\Bbb{Q} + \\R`} и ${$`\\R + \\Bbb{Q}`}.)


${start(area.key('teor_14'))}
${b`Теорема 14`}. Всякое счётное линейно упорядоченное множество изоморфно некоторому подмножеству множества ${$`\\Bbb{Q}`}.
${end(area)}
...
Доказательство этого утверждения происходит так же, как и в ${label.ref('teor_13')`теореме 13`} — с той разницей, что новые необработанные элементы берутся только с одной стороны (из данного нам множества), а пары к ним подбираются в множестве рациональных чисел.
${$`${teor_end}`}


${problem(api)} Дайте другое доказательство ${label.ref('teor_14')`теоремы 14`}, заметив, что любое множество ${X} изоморфно подмножеству множества ${$`\\Bbb{Q} \\times X`}.
`;
};

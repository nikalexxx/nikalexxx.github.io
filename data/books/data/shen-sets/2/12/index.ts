...
export default (api) => {
    ...
    return _`
${paragraph('2.12. Индуктивные определения и степени')}
...
${start(area.key('teor_38'))}
${b`Теорема 38`}. Сложение ординалов обладает следующими свойствами:
${$$`
\\begin{aligned}
\\alpha + 0 &= \\alpha; \\\\
...
${start(area.key('teor_39'))}
${b`Теорема 39`}. Умножение ординалов обладает следующими свойствами:
${$$`
\\begin{aligned}
\\alpha 0 &= 0; \\\\
...
\\end{aligned}
`}
${label.ref('teor_18')`Теорема 18`} (о трансфинитной рекурсии) гарантирует, что эти соотношения однозначно определяют некоторую операцию над ординалами, которая и называется возведением в степень.
...
${b`Теорема 40`}. Указанное правило задаёт полный порядок на множестве ${$`[B \\to A]`} и порядковый тип этого множества есть ${$`\\alpha^{\\beta}`}.
${end(area)}
...
${start(area.key('teor_41'))}
${b`Теорема 41`}. Если ${$`\\alpha`} и ${$`\\beta`} — счётные ординалы, то ${$`\\alpha + \\beta`}, ${$`\\alpha\\beta`} и ${$`\\alpha^\\beta`} счётны.
${end(area)}
...
${problem(api)} Докажите, что ${$`\\alpha^{\\beta + \\gamma} = \\alpha^{\\beta} \\cdot \\alpha^{\\gamma}`} двумя способами: по индукции и с использованием явного определения степени.


${problem(api)} Докажите, что ${$`(\\alpha^{\\beta})^{\\gamma} = \\alpha^{\\beta\\gamma}`}.


${problem(api)} Докажите, что если ${$`\\alpha > 2`}, то ${$`\\alpha^{\\beta} > \\alpha\\beta`}.


${problem(api)} Докажите, что если ${$`\\omega^{\\gamma} = \\alpha + \\beta`} для некоторых ординалов ${$`\\alpha, \\beta`} и ${$`\\gamma`}, то либо ${$`\\beta = 0`}, либо ${$`\\beta = \\omega^{\\gamma}`}.


${problem(api)} Какие ординалы нельзя представить в виде суммы двух меньших ординалов?


${problem(api)} Докажите счётность ${$`\\alpha^{\\beta}`} для счётных ${$`\\alpha`} и ${$`\\beta`}, используя индуктивное определение степени.


${problem(api)} Дан некоторый ординал ${$`\\alpha > 1`}.
...
${start(area.key('teor_42'))}
${b`Теорема 42`}. Всякий ординал, меньший ${$`\\alpha^{\\beta}`}, представляется в виде
${$$`
...
Это утверждение обобщает описанную нами ранее (${draft`с. 90`}) «позиционную систему обозначений с основанием ${$`\\alpha`}» для ординалов, меньших ${$`\\alpha^k`}; теперь вместо ${$`k`} можно использовать любой ординал.
...
${problem(api)} Докажите, что
${$$`
\\varepsilon_0 = \\omega + \\omega^{\\omega} + \\omega^{(\\omega^{\\omega})} + \\ldots
`}


${problem(api)} Определим для натуральных чисел операцию «тотальной замены основания ${$`k`} на ${$`l`}»
